...
const { InformationStatus,Student,Information,Admin } = require('../../models')
//模糊搜索需要
...
router.get('/', async function (req, res, next) {
  try {
    ...
    const condition={
      where:{studentId:id},
      attributes:{exclude:['informationId','studentId']},
      include:[{
        model:Information,
        as:'information',
    ...
    success(res,'查询学生信息状态详情成功',{informationstatuses})
  } catch (error) {
    failure(res,error)
  }
})
...
router.put('/:id', async function (req, res) {
  try {
    ...
    await informationstatuse.update({status:1})
...
async function getInformationStatus(req) {
  ...
  //查询当前学生信息状态
  const informationstatuse = await InformationStatus.findByPk(id,condition)

  //如果没有找到就抛出异常
  if (!informationstatuse) {
    throw new NotFound(`ID:${body.studentId}的学生信息状态未找到`)
  }
